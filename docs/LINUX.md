# Preparation and Deployment on Linux (Ubuntu)

This file describes the processes for preparing and deploying the application on the Linux platform, using Ubuntu as an example.
...
Install via apt (if not installed):
...
curl -fsSL https://releases.hashicorp.com/packer/$(curl -s https://checkpoint-api.hashicorp.com/v1/check/packer | jq -r '.current_version')/packer_$(curl -s https://checkpoint-api.hashicorp.com/v1/check/packer | jq -r '.current_version')_linux_amd64.zip -o packer.zip
...
**Step 5: Configure running without sudo (optional)**
...
More details: [https://docs.docker.com/engine/install/ubuntu/](https://docs.docker.com/engine/install/ubuntu/)

### qwen-code

Installing qwen-code on the host system (Linux):
...
You are now ready to work with DevCage on Linux (Ubuntu).
