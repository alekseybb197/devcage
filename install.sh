...
if [ $? -eq 0 ]; then
  ...
  # Find the most recent qwen-code image tag
...
EOF
    # Copy qode script to /usr/local/bin for global access
    if [ -f "./qode" ]; then
      sudo cp "./qode" /usr/local/bin/qode
      sudo chmod +x /usr/local/bin/qode
      echo "Copied qode to /usr/local/bin/qode"
    else
